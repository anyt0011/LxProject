...
static int32_t Receive_Byte(uint8_t *c, uint16_t length, uint32_t timeout)
{
  ...
  if (xSemaphoreTake(binarySemaphore, timeout) == pdTRUE) 
  {
    return 0;
  }
  return 1;
...
/**
  ...
  *         1: abort by user
  */
extern uint16_t receive_data_len;
static int32_t Receive_Packet (uint8_t *data, int32_t *length, uint32_t timeout)
{
  uint16_t packet_size;
  *length = 0;
  if (Receive_Byte(data,BUFFER_SIZE,timeout) != 0)
  {
    return -1;
  }
  ...
  //校验搬运长度和接收长度是否一致
  if((receive_data_len + packet_size + PACKET_OVERHEAD) != BUFFER_SIZE)
  {
    return -1;
  }
              ...
              else
              {
                if (packets_received == 0)
                {
                  ...
                  if (packet_data[PACKET_HEADER] != 0)
                  {
                    ...
                    //TODO:通知，不做任何处理
                    xQueueSend(newappdataQueue, &size, 0);
                    if(packet_data == buf1)
                    {
                      packet_data = buf2;
                    }
                ...
                {
                  xSemaphoreTake(w25qstateSemaphore,portMAX_DELAY);
                  //1.数据缓存区地址
                  //2.数据长度
                  s32_W25qDataSize = packet_length;
									packet_data += PACKET_HEADER;
                  xQueueSend(newappdataQueue, &packet_data, 0);
                  packet_data -= PACKET_HEADER;
                  //W25Q64_WriteData(buf,packet_length);
                  if(packet_data == buf1)
                  {
                    packet_data = buf2;
                  }
                  else
                  {
                    packet_data = buf1;
									...
									xSemaphoreGive(w25qstateSemaphore);//////////////////////////////////改过
                  Send_Byte(ACK);
                }
                packets_received ++;
                session_begin = 1;
